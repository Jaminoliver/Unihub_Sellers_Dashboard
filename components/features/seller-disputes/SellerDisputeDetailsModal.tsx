...
interface SellerDisputeDetailsModalProps {
...
export function SellerDisputeDetailsModal({ dispute, onClose }: SellerDisputeDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <Card className="max-w-5xl w-full h-[90vh] flex flex-col">
        ...
        <CardHeader className="border-b bg-gradient-to-r from-orange-600 to-orange-500 text-white flex-shrink-0 py-1.5 px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <MessageSquare className="h-4 w-4" />
              <div>
                <CardTitle className="text-sm font-semibold leading-tight">Dispute</CardTitle>
          ...
          {isLoading ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <div className="animate-spin rounded-full h-10 w-10 border-4 border-orange-500 border-t-transparent mx-auto mb-2"></div>
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center max-w-md">
                ...
                <h3 className="font-semibold text-gray-900 mb-1 text-sm">Waiting for admin</h3>
                    ...
                    <div className={`flex ${isYou ? 'justify-end' : 'justify-start'}`}>
                      <div className={`flex items-end gap-1.5 max-w-[75%] ${isYou ? 'flex-row-reverse' : 'flex-row'}`}>
                        <div className={`h-6 w-6 rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 ${
                          isYou ? 'bg-green-500' : 'bg-purple-500'
                        }`}>
                          {isYou ? 'Y' : 'A'}
                        </div>
                        ...
                        <div>
                          <div className={`rounded-xl px-3 py-1.5 ${
                            isYou ? 'bg-green-500 text-white' : 'bg-white border border-gray-200'
                          }`}>
                            {!isYou && (
                              <p className="text-xs font-semibold mb-0.5 text-purple-900">Admin</p>
                            )}
                                      ...
                                      {isPDF ? (
                                        <a 
                                          ...
                                          className={`flex items-center gap-1.5 p-1.5 rounded text-xs ${
                                            isYou ? 'bg-green-600' : 'bg-purple-50'
                                          }`}
                                        >
                                          <FileText className={`h-3 w-3 ${isYou ? 'text-white' : 'text-purple-900'}`} />
                                          <span className={`${isYou ? 'text-white' : 'text-purple-900'} truncate`}>
                                            {getFileName(url)}
                                          </span>
                                        </a>
                                      ) : (
          ...
          <div className="border-t bg-white p-2 flex-shrink-0">
            {selectedFiles.length > 0 && (
              <div className="mb-2 flex flex-wrap gap-1.5">
                {selectedFiles.map((file, index) => (
                  <div key={index} className="bg-orange-50 border border-orange-200 rounded px-2 py-1 flex items-center gap-1.5">
                    {file.type.startsWith('image/') ? <ImageIcon className="h-3 w-3 text-orange-600" /> : <FileText className="h-3 w-3 text-orange-600" />}
                    <span className="text-xs text-orange-900 max-w-[100px] truncate">{file.name}</span>
                    <button onClick={() => removeFile(index)} className="text-orange-600 hover:text-orange-800">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
            ...
            <div className="flex items-end gap-2">
              ...
              <textarea
                ...
                className="flex-1 p-2 border rounded-lg resize-none focus:ring-2 focus:ring-orange-500 text-sm"
                rows={1}
                disabled={isSending}
              />
              <Button
